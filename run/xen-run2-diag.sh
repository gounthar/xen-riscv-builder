...
# DIAG: trace the block hotplug script to a file, so the line it hangs on survives the kill.
send 13t blocktrace "sed -i '1a exec 2>>/var/log/xen/block-trace.log; set -x' /etc/xen/scripts/block"
send 13u blockhead  'sed -n 1,3p /etc/xen/scripts/block'
...
printf 'echo ---BLOCK-TRACE---\n'
printf 'tail -60 /var/log/xen/block-trace.log 2>&1\n'
printf 'echo ---BLOCK-TRACE-END---\n'
printf 'ls -l /dev/loop* 2>&1 | head; losetup -a 2>&1; ls -la /var/run/xen-hotplug 2>&1\n'
printf 'ps 2>&1 | tail -30\n'
printf 'echo ---DIAG-END---\n'
