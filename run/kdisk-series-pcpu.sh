#!/usr/bin/env bash
# COPY of kdisk-series.sh for exp 6+7 (Xen 4307f2165ce4), 2026-09-23: also stops a run on a Xen assertion instead of waiting for the 5400 s cap.
...
one() {   # one <N> <multinode 0|1>
	...
	while :; do
		...
		grep -aq -E "Kernel panic|Unhandled exception|Assertion .* failed" $L 2>/dev/null && break
