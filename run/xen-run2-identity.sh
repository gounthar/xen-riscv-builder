...
# IDENTITY (fedora1-only copy, run 67): no test, only the payload's
# test=identity probe. Keeps the disk stanza so blkfront attaches.
send 12i ident "sed -i 's/test=all/test=identity/' /domu/domu.cfg"
send 12j identchk 'grep -c test=identity /domu/domu.cfg'
