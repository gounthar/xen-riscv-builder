#!/usr/bin/env bash
# Interleaved A/C series for the per-cpu assertion, 2026-09-25 (runs 137+). Copy of ab-series.sh.
# A = base Xen 08074f7f22ee, B arm = C = base + RFC 0012 percpu.h hunk only, fd4a2057216b (no place_modules).
...
set -u
R=$HOME/xen-run; IMG=$R/initrd-tools-kdisk.img; OUT=$R/ac-series.log
XEN=$R/xen/xen/xen
A_BIN=$R/xen-bin-pgoar-08074f7f22ee; A_SHA=08074f7f22ee78bdc54acf7503729240cb08d5d459c381107fd527b2ecce64dc
B_BIN=$R/xen-bin-pcpuonly-fd4a2057216b; B_SHA=fd4a2057216bc827d49fd7ee4126acbc4cca6a8fc49d403945aea44f6a84466e
...
echo "AC_SERIES_START $(date -Is) first=$N cutoff=$(date -d @$CUTOFF -Is)" >> $OUT
k=0
while :; do
...
install_bin A >/dev/null && echo "AC_SERIES_DONE $(date -Is) pairs=$k installed=$(sha256sum $XEN | cut -c1-12)" >> $OUT
